...
namespace Lean.Pool
{
	...
	public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public bool RemoveDelayDespawn(GameObject clone)
		{
			for (var i = delays.Count - 1; i >= 0; i--)
			{
				var delay = delays[i];
		...
		private Dictionary<int,IOnDespawn[]> allOnDespawnes;//销毁
		private Dictionary<int,IOnSpawn[]> allOnSpawnes;//出现
		private void InvokeOnSpawn(GameObject clone)
		{
			...
			//运行该物体上所有OnSpawn
			if (allOnSpawnes == null)allOnSpawnes = new Dictionary<int, IOnSpawn[]>();
			var instanceid = clone.GetInstanceID();
			if (!allOnSpawnes.ContainsKey(instanceid))
			{
				allOnSpawnes.Add(instanceid,clone.GetComponents<IOnSpawn>());//添加
			}
			var getIonspawnes = allOnSpawnes[instanceid];
			foreach (var onspawn in getIonspawnes)
			{
				onspawn.OnSpawn();
				onspawn.onSpawn?.Invoke();
				onspawn.onSpawnClear?.Invoke();
				onspawn.onSpawnClear = null;
			}
		}
		...
		private void InvokeOnDespawn(GameObject clone)
		{
			...
			if (allOnDespawnes == null)allOnDespawnes = new Dictionary<int, IOnDespawn[]>();
			var instanceid = clone.GetInstanceID();
			if (!allOnDespawnes.ContainsKey(instanceid))
			{
				allOnDespawnes.Add(instanceid,clone.GetComponents<IOnDespawn>());//添加
			}
			var getIdespawnes = allOnDespawnes[instanceid];
//			Debug.Log($"有多少Ondespawn:{getIdespawnes.Length}");
			foreach (var despawn in getIdespawnes)
			{
				despawn.OnDespawn();
				despawn.onDespawn?.Invoke();
				despawn.onDespawnClear?.Invoke();
				despawn.onDespawnClear = null;
			}
		}
